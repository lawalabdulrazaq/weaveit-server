...
export async function ensureUser(walletAddress) {
    await pool.query(`INSERT INTO users (wallet_address) 
     VALUES ($1) 
     ON CONFLICT (wallet_address) DO NOTHING`, [walletAddress]);
}
...
export async function getVideosByWallet(walletAddress) {
    const result = await pool.query(`SELECT video_id, job_id, duration_sec, format, created_at 
     FROM videos 
     WHERE wallet_address = $1 
     ORDER BY created_at DESC`, [walletAddress]);
    return result.rows;
}
